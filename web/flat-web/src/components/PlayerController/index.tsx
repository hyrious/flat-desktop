...
import video_pauseSVG from "./icons/video_pause.svg";
import video_playSVG from "./icons/video_play.svg";
...
> {
    ...
    private operationButton = (phase: PlayerPhase): React.ReactNode => {
        switch (phase) {
            case PlayerPhase.Playing: {
                return <img src={video_pauseSVG} />;
            }
            case PlayerPhase.Buffering: {
                return <LoadingOutlined style={{ fontSize: 14, color: "#7A7B7C" }} />;
            }
            default: {
                return <img style={{ marginLeft: 2 }} src={video_playSVG} />;
            }
        }
    };

    private handleMultiplePlay = (multiple: number): void => {
        const { player } = this.props;
                    ...
                    <div className="player-controller-mid">
                        <div className="player-left-box">
                            ...
                            <div className="player-mid-box-time">
                                {renderTime(Math.floor(player.progressTime / 1000))} /{" "}
                                {renderTime(Math.floor(player.timeDuration / 1000))}
                            </div>
                        </div>
                        <Dropdown overlay={this.renderMultipleSelector} placement="topCenter">
                            <div className="player-right-box">
                                {this.state.multiple === 1.0
                                    ? this.props.i18nLanguage === "zh-CN"
                                        ? "倍速"
                                        : "speed"
                                    : `${this.state.multiple}x`}
                            </div>
                        </Dropdown>
                    </div>
                </div>
